// ============================================
// FOXBITE LEARNING - ULTIMATE PREMIUM JS
// With Responsive Image Detection
// ============================================
...
function detectDevice() {
    ...
    console.log(`Loading: ${isMobile ? 'mobile.png' : 'student.png'}`);
}
...
// ============================================
// COURSE CARD TOUCH FOR ENROLL BUTTON FLOAT
// ============================================
    ...
    const observer = new IntersectionObserver((entries) => {
        entries.forEach(entry => {
            if (entry.isIntersecting && !entry.target.classList.contains('counted')) {
                ...
                const label = entry.target.closest('.stat-item').querySelector('.stat-label').textContent;
...
if (heroImage) {
    heroImage.addEventListener('error', function() {
        ...
        console.log(`Please ensure ${imageName} is in the same folder as index.html`);
        
        this.style.display = 'none';
        const container = this.closest('.hero-image-container');
        if (container) {
            container.style.background = 'linear-gradient(135deg, #1e3a8a 0%, #1e40af 100%)';
            container.innerHTML += `<div style="position: absolute; top: 50%; left: 50%; transform: translate(-50%, -50%); text-align: center; color: white; font-size: 1.5rem; font-weight: 700; padding: 2rem; z-index: 1;">
                ${imageName} Not Found<br>
                <small style="font-size: 0.875rem; opacity: 0.7; margin-top: 0.5rem; display: block;">
                    Place ${imageName} in the same folder
                </small>
            </div>`;
        }
    });
...
window.addEventListener('load', () => {
    ...
    console.log('%cUltimate Premium Education Platform', 'color: #a0a0a0; font-size: 13px;');
    console.log('%c✓ Responsive image system active', 'color: #10b981; font-size: 11px;');
    console.log('%c✓ Desktop: student.png | Mobile: mobile.png', 'color: #10b981; font-size: 11px;');
