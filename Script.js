...
// ============================================
// COURSE CARD TOUCH/HOVER FOR ENROLL BUTTON FLOAT
// ============================================
...
courseCards.forEach(card => {
    ...
    card.addEventListener('touchend', function() {
        // Keep the touched class for a while
        setTimeout(() => {
            this.classList.remove('touched');
        }, 3000);
    });
    
    // For mouse devices - hover handles it via CSS
});
...
if (heroImage) {
    heroImage.addEventListener('error', function() {
        ...
        console.log('Please ensure student.png is in the same folder as index.html');
        this.style.display = 'none';
        const wrapper = this.closest('.hero-image-wrapper');
        if (wrapper) {
            wrapper.style.background = 'linear-gradient(135deg, #1e3a8a 0%, #1e40af 100%)';
            wrapper.innerHTML = '<div style="position: absolute; top: 50%; left: 50%; transform: translate(-50%, -50%); text-align: center; color: white; font-size: 1.5rem; font-weight: 700; padding: 2rem;">Student Image<br><small style="font-size: 0.875rem; opacity: 0.7; margin-top: 0.5rem; display: block;">Place student.png in the same folder</small></div>';
        }
    });
...
window.addEventListener('load', () => {
    ...
    console.log('%c✓ FAQ mobile compatibility fixed', 'color: #10b981; font-size: 11px;');
    console.log('%c✓ Course hover animations enabled', 'color: #10b981; font-size: 11px;');
    console.log('%c✓ Scroll indicator active', 'color: #10b981; font-size: 11px;');
});
...
// Prevent default touch behavior on course cards for better UX
